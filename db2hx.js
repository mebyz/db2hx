...
var Main = function() { };
Main.__name__ = ["Main"];
Main.getForeignKeysQuery = function(tableName,schemaName) {
	return "SELECT \n            ccu.table_name AS source_table \n            ,ccu.constraint_name AS constraint_name \n            ,ccu.column_name AS source_column \n            ,kcu.table_name AS target_table \n            ,kcu.column_name AS target_column \n          FROM INFORMATION_SCHEMA.CONSTRAINT_COLUMN_USAGE ccu \n          INNER JOIN INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS rc \n              ON ccu.CONSTRAINT_NAME = rc.CONSTRAINT_NAME \n          INNER JOIN INFORMATION_SCHEMA.KEY_COLUMN_USAGE kcu  \n              ON kcu.CONSTRAINT_NAME = rc.UNIQUE_CONSTRAINT_NAME \n              WHERE ccu.table_name = " + js_npm_sequelize_Sequelize.Utils.addTicks(tableName, "'");
};
Main.loadConfiguration = function(pEnvFile) {
	console.log("setting env file to : " + pEnvFile);
	var confPath = pEnvFile;
	var confFile = js_node_Fs.readFileSync(confPath,"utf8");
	return yaml_Yaml.parse(confFile);
};
Main.initDb = function(conf) {
	var dbOpts = conf.get("DB_OPTIONS");
	var poolOpts = dbOpts.get("pool");
	var dialectOps = dbOpts.get("dialectOptions");
	var opts = { host : dbOpts.get("host"), dialect : dbOpts.get("dialect"), storage : dbOpts.get("storage"), pool : { max : poolOpts.get("max"), min : poolOpts.get("min"), idle : poolOpts.get("idle")}, dialectOptions : { encrypt : dialectOps.get("encrypt")}, logging : dbOpts.get("logging")};
	return new js_npm_sequelize_Sequelize(conf.get("DB_NAME"),conf.get("DB_USER"),conf.get("DB_PASSWORD"),opts);
};
Main.main = function() {
	var async = require("async");
	if(Reflect.hasField(process.env,"output") && Reflect.field(process.env,"output") != "" && Reflect.hasField(process.env,"conf") && Reflect.field(process.env,"conf") != "") {
		var conf = Main.loadConfiguration(Reflect.field(process.env,"conf"));
		var db = Main.initDb(conf);
		var qi = db.getQueryInterface();
		console.log(db.options.dialect);
		qi.showAllTables().then(function (t) { 
	...
	} else console.log("missing one or more parameters ( usage : conf=[conf_filename] output=[output_folder] ./run.sh ) ");
};
Math.__name__ = ["Math"];
...
var js_npm_sequelize__$Transaction_C_$LOCKS = function() { };
js_npm_sequelize__$Transaction_C_$LOCKS.__name__ = ["js","npm","sequelize","_Transaction","C_LOCKS"];
js_npm_sequelize__$Transaction_C_$LOCKS.prototype = {
	__class__: js_npm_sequelize__$Transaction_C_$LOCKS
};
